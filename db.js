...
function queryExactlyOne(q, values) {
  return query(q, values).then(results =>
    results.length === 1 ?
      results[0] :
      Promise.reject(new Error('Database: row not found')))
}
...
module.exports = { get, query, queryOne, queryExactlyOne, insert }
